...
public interface IPrivilegedUserService
{
    ...
    Task<PageResult<JWTAuthentication.DTOs.UserInfoDTO>> GetUsersAsync(GetUsersPaginationViewModel model);
}
...
public class PrivilegedUserService : IPrivilegedUserService
{
    ...
    public async Task<ServerResponse> BlockAsync(string userId) // DONE. 
    {
        ...
        {
            Message = $"{imageDirectory}Д{userName}Д{user.Email}",
            IsSuccess = true
        };
    }
    ...
    public async Task<PageResult<JWTAuthentication.DTOs.UserInfoDTO>> GetUsersAsync(GetUsersPaginationViewModel model)
    {
        ...
        List<JWTAuthentication.DTOs.UserInfoDTO> users = await query.Select(u => new JWTAuthentication.DTOs.UserInfoDTO
        {
            Id = u.Id,
            UserName = u.UserName
        }).ToListAsync();

        foreach (JWTAuthentication.DTOs.UserInfoDTO u in users)
        {
        ...
        return new PageResult<JWTAuthentication.DTOs.UserInfoDTO>
        {
            Count = totalCount,
